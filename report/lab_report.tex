...
a fixed representation, and then another recurrent network is uded to decode the representation into a sequence \cite{s2s}.
Srivastava \emph{et al.} use the LSTM Encoder-Decoder framework extending the general framework to learn video representations \cite{ulvr}.
...
the gate layers can look at the cell state. Functions $\sigma$, $g$ and 
$h$ are pointwise nonlinear activation functions. $\mathbf{i}$, $\mathbf{f}$, $\mathbf{o}$ are separately input gate, forget gate, and
output gate. The pointwise multiplication of two vectors is denoted with $\odot$:
